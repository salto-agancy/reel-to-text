...
- `/stats`: только для админов, размер кэша и число платных запросов за сутки
...
- Доступ только по списку ID, пока `ACCESS_MODE=allowlist`. Чужой пользователь получает свой ID и просьбу отправить его владельцу.
